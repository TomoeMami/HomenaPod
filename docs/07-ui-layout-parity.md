...
| 单集详情 `EpisodeDetailPage`（第 52 轮新增） | `feeditem_fragment.xml` + `ItemFragment.java`：头部（56dp 封面 + 订阅名 + 标题 maxLines=5 + 时长 · 日期）+ 两个 `butAction`（播放/流播/暂停、下载/取消/删除，含 40dp 进度环）+ WebView shownotes | 同构：`AppBar`（标题 = 订阅名，本集为当前播放项时多一个「打开播放页」）+ 56vp 封面（点击进订阅详情）+ 标题 5 行 + 「时长 · 发布日期」+ 主/次操作（各占一半、40vp 进度环）+ 纯文本正文（保留换行、时长内时间码可点）；入口 = 收件箱单击 |
...
| 7 | 播放页节目详情（上游 WebView 渲染 HTML shownotes） | **替代实现（第 52 轮补换行与时间码）** | 无 Web 组件依赖，用 `HtmlCleaner.stripHtml` 纯文本降级：`<br>`/`<p>`/`h1`~`h5`/`tr`/`div`/`li` 都转成换行（上游 `HtmlToPlainText` 的规则），正文里落在时长内的时间码渲染成可点段（上游 `ShownotesCleaner.addTimecodes` + `Converter.durationString*ToMs`，点按 seek），见 `utils/ShownotesText.ets` / `components/ShownotesBody.ets`。**旧实现把标签换成空格后压缩 `\s+`，换行被整体吃掉**。链接/图片仍是纯文本（不重建 `<a>` 的裸链接）。 |
...
| 35 | 点击行打开单集详情页（`ItemPagerFragment`） | **已同步（第 52 轮，用户选择；仅收件箱）** | 新增 `pages/EpisodeDetailPage.ets`（对照 `feeditem_fragment.xml` + `ItemFragment.java`），收件箱单击行改为进详情页（不再直接播放），播放由详情页主操作 / 行内次级按钮触发。其余列表（订阅详情 / 单集总表 / 收藏 / 搜索 / 历史 / 队列 / 首页）仍是「单击直接播放 + 跳播放页」（第 41 轮选择不变）。上游的左右滑动换集（`ViewPager2`）与「上游由 Activity 提供的收藏/睡眠工具栏」未做；播放后留在详情页（上游行为），因本移植版路由页没有迷你播放条，补了标题栏「打开播放页」按钮。 |
