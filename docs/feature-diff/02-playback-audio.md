...
4. **倍速档位**：移植版 7 档离散、上限 2.0（`PlayerManager.ets:38`，注释称 3.0x 需 API 13）；上游滑杆连续 0.5–4.0（`PlaybackSpeedSeekBar.java:69-82` + `playback_speed_seek_bar.xml` max=70）。**第 56 轮起播放页改速是弹层选择**（`PlayerPage.ets` 的 `speedSheet()` / `applySpeed()`，对照上游 `VariableSpeedDialog.java:120-128`）：选择后立即作用于当前播放并写全局默认，弹层里显示当前默认值；旧实现是「点一次循环一档、并顺手改写全局默认」，且 2.00x 再点会绕回 0.50x（误触即静默把默认倍速改成 0.5x）。临时倍速那一层（`PlaybackPreferences.java:155-159`）仍未移植。
...
13. **「断的是不是蓝牙」要靠应用自己记一份输出设备**（第 55 轮，移植版独有特性）：设备变更事件带回的 `devices` 是**变更之后**的设备列表（SDK：「Audio device descriptors after change」），蓝牙断开时那里只剩扬声器，单看这一次事件答不出「下线的是什么设备」；`preDevices`（变更前列表）是 API 26 才有的字段，本工程 compatibleSdkVersion=14 用不了。因此 `PlayerManager` 在 `play()` 时用 `AudioRoutingManager.getPreferredOutputDeviceForRendererInfoSync(AudioRendererInfo)` 采样一次「本流此刻用的输出设备」，并在每次设备变更事件后更新；设备丢失事件到达时比较「变更前有蓝牙、变更后没有」即认定为蓝牙下线。**采样用的是按 `usage` 的场景级查询，不是全局 `AudioRoutingManager.on('deviceChange')`** —— 官方 FAQ 明确全局事件监听的是所有设备的连接状态、不与音频流绑定，不能作为自动暂停之类判断的依据。判定不出来（历史为空 / 变更后仍是蓝牙）→ 不回退：宁可漏一次，也不要在拔有线耳机或切扬声器时回退几秒。**上机取证**（模拟器，脚手架喂事件）：蓝牙臂 `bluetoothLost=true rewind=5000ms` → 暂停后 `22299 -> 17299`；有线臂 `bluetoothLost=false rewind=0ms` → 位置原样不动；停在暂停态读设备库，`FeedMedia.position` 正是回退目标（`.dsh-out/r55/hilog-verify.txt`）。**真机仍未覆盖**：模拟器无法配对蓝牙耳机，故「系统真的派发 `REASON_OLD_DEVICE_UNAVAILABLE` 且变更后列表里没有蓝牙」仍只有官方文档依据。
14. **所有播放入口共用一份 `Playable` 构造**（第 56 轮修）：倍速/跳片头/跳片尾/音量适配都属于**每订阅设置**，只有 `QueueEngine.toPlayable()` 会读它们。此前 `EpisodesPage.play()` 自己 hand-build 了一个 `Playable` 并把 speed 写死 1.0、其余三项写死 0 —— 于是「单集列表行内按钮播放已下载单集」会丢掉订阅级倍速、落到**全局默认倍速**；而列表里未下载单集的按钮是「下载」不是播放（`EpisodesPage.ets` 的 `onAction` 分支），想在线听只能进单集详情页，那条路读得到订阅级倍速。结果就是同一集**下载的比在线的慢**（实测：feed 1.5x + 全局 1.0x 时，行内按钮实测 0.995x，详情页原生 `AudioSinkFilter::SetSpeed speed is 1.500`）。现在该入口与其它页面一样走 `QueueEngine.toPlayable()`；同一入口复测 1.484x。
15. **倍速是否真的生效可自检**（第 56 轮，移植版独有）：`player.setSpeed()` 是异步的、平台会丢弃过早的下发，而旧实现既不校验也不留日志（界面显示 1.50x / 实际播 1.00x 完全不可见）。现在 `PlayerManager` 订阅 `speedDone`（回传**实际生效**档位）并与请求档位比对，不一致就补发一次（`handleSpeedDone()`；进入 `playing` 时再兜底补发一次），日志固定为 `setSpeed src=… request=… mode=…` + `speedDone effective=… expected=…`；系统播控下发的倍速先过 `PlaybackSpeedPolicy`（`player/PlaybackSpeedPolicy.ets`，纯逻辑 + `runtime-pure-test.ps1` 断言）：非有限数值、或不在 `SPEED_PRESETS` 区间内的一律拒绝——因为 `toPlaybackSpeed()` 用 `<=` 分档，任何 ≤0.5 的输入都会静默落到 0.5x。
...
6. **跳片头在 `load()` 内直接抬升起点**（`PlayerManager.ets:93-99`），跳片尾用 `timeUpdate` 一次性触发（`PlayerManager.ets:312-327`）。
7. **自检接口**：`getVolume()`（`PlayerManager.ets:256-258`）、`isListening()`（`ShakeDetector.ets:44-46`）、`isRunning()`（`BackgroundPlaybackGuard.ets:59-61`）。
8. **蓝牙断开后的补偿回退**（第 55 轮，`OutputDevicePolicy.ets:138-164` + `PlayerManager.ets:311-350`）—— 上游没有：蓝牙掉线到系统通知应用之间有延迟，暂停的同时把进度往回拉 5 秒（默认，设置页可改），重连后不漏内容；只对蓝牙生效（靠应用自己记录的输出设备类别判定）。
