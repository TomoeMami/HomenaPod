...
2. **中断恢复**：上游三项独立偏好（`preferences_playback.xml:5-27`）→ 移植版合并为 `prefAutoResumeAfterInterrupt`（`UserPreferences.ets:214-221`），恢复路径仅为系统 `audioInterrupt` 的 `INTERRUPT_HINT_RESUME`（`PlayerManager.ets:340-344`）。**若系统不派发 RESUME（部分蓝牙重连场景），则永不恢复**，且耳机与蓝牙无法分别配置。
