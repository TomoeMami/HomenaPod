...
<details>
<summary>附：开发工作区说明（本地协作与克隆）</summary>

本仓库同时是一个**开发工作区**：`PLAN.md` + `docs/` 保存了移植计划、任务清单与上游对照依据，可由执行者（人或智能体）按 `docs/02-task-list.md` 从 `T0.1` 顺序执行。
...
局域网 Git 克隆（git+ssh，本机 daemon 9418 已停用）：
...
> 克隆设备需已把公钥加入本机 `~/.ssh/authorized_keys`。
